...
| POST | `/api/v1/paths/{path_id}/confirm` | Sí | Confirma el esquema revisado: reordenar, renombrar, eliminar temas y fijar `coverage_policy`. Si la política final es `source_only`, además quita del esquema ya persistido los temas que sigan `insufficient` (y el módulo entero si se queda sin ninguno) — la Fase A resolvió con la política por defecto, antes de que el aprendiz eligiera; esto es lo que hace real su elección. Con `request_more` no se bloquea nada todavía: el tema se conserva y la Fase B lo sigue completando con saber del modelo, igual que `model_knowledge` — pendiente de una política de espera real. Encola el módulo 1. | `PathDetailOut` |
