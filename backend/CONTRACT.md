...
| Método | Ruta | Auth | Descripción | Respuesta |
|---|---|---|---|---|
| GET | `/api/v1/paths` | Sí | Mis rutas + Rutas del Reino (P22). Filtro `scope=mine|seed|all`. Cada fila lleva `mastery`: el dominio del **Conocimiento** del que cuelga la Ruta (`UserAreaProgress.mastery`, no una columna propia de la Ruta), que es lo que pinta el medallón «Dominio» de la tarjeta. | `Page<PathSummaryOut>` |
...
| GET | `/api/v1/paths/{path_id}` | Sí | Mapa de la ruta (P07): módulos, temas, lecciones y estado de bloqueo por usuario. Cada nodo de **tema** lleva su `coverage`, que es lo único con lo que el cliente distingue «esto sale de tu material» de «esto lo escribió el Reino» —de ahí la etiqueta «Saber del Reino»—, y la respuesta lleva `coverage_notes` con los temas que el material no cubre, que es lo que la pantalla de generación lista al pedir la política. Cada nodo de módulo lleva su `summary` y sus `estimated_minutes` —que el mapa pinta como párrafo y como píldora de duración— y su `content_status`: sin él el cliente lo lee nulo, cae a `pending` y pinta **todos** los módulos «En construcción». Y lleva además su desafío en `assessment` —`null` mientras el módulo no tenga evaluación creada—: `{assessment_id, module_id, title, question_count, pass_score, max_attempts_per_day, content_status, attempts_used, best_score, passed, can_start, cooldown_until}`. `can_start` mira **solo** las reglas del desafío —enfriamiento y tope diario (§5.5, §8.6)—; que el módulo esté bloqueado lo dice `status` del nodo, y mezclarlos dejaría al mapa sin poder distinguir «bloqueado» de «hoy ya no te quedan intentos». La respuesta lleva también `mastery` en la raíz (espejo de `path.mastery`, igual que `completion_pct`): el dominio del Conocimiento del que cuelga la Ruta, para el medallón «Dominio del territorio» de la cabecera. | `PathDetailOut` |
| POST | `/api/v1/paths/{path_id}/confirm` | Sí | Confirma el esquema revisado: reordenar, renombrar, eliminar temas y fijar `coverage_policy`. Si la política final es `source_only`, además quita del esquema ya persistido los temas que sigan `insufficient` (y el módulo entero si se queda sin ninguno) — la Fase A resolvió con la política por defecto, antes de que el aprendiz eligiera; esto es lo que hace real su elección. Si la política final es `request_more` y todavía queda algún tema `insufficient`, el confirm **no avanza**: responde `409 MATERIAL_PENDING`, la ruta se queda en `PENDING_REVIEW` (no pasa a `GENERATING`, no se encola el módulo 1) y la elección de política junto con los `cambios` ya enviados sí quedan guardados. Reintentar el confirm — típicamente tras subir más material con `POST /documents` — vuelve a evaluar; si el aprendiz prefiere no esperar, puede reconfirmar con otra política. **No existe hoy una regeneración automática de los temas ya persistidos** al subir material nuevo — la Fase A no se puede volver a invocar sobre un tema puntual —, así que subir un documento no basta por sí solo: el aprendiz tiene que editar manualmente esos temas por `cambios`, o cambiar de política. De lo contrario, encola el módulo 1. | `PathDetailOut` |
...
| Método | Ruta | Auth | Descripción | Respuesta |
|---|---|---|---|---|
| GET | `/api/v1/modules/{module_id}/assessment` | Sí | Pantalla de entrada (P11): reglas, recompensa, intentos usados y enfriamiento. | `AssessmentInfoOut` `{assessment, attempts_used, cooldown_until, can_start, reward_preview, topic_titles, assessment_attempts}`. `topic_titles` son los temas del módulo, en orden, para la sección «Qué entra»: sin ellos esa sección no tiene nada que pintar. `assessment_attempts` es `UserModuleProgress.assessment_attempts` (histórico de por vida, sobrevive a que el banco se regenere) — distinto de `attempts_used`/`attempts_total`, que cuentan solo la evaluación vigente. |
...
| GET | `/api/v1/streak` | Sí | Racha actual, mejor, estado, próximo hito y su recompensa (P18). | `StreakOut` `{current, best, status, total_active_days, grace_available, next_milestone{days, remaining, reward}, previous_length, last_change, started_on}` |
